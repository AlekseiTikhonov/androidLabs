...
//далее из полученого массива переносим данные в список элементов
                ...
                putDataToDB();
                ComponentName watchWidget; //объект для виджета
                watchWidget = new ComponentName(this, NewAppWidget.class); //связываем с нашим виджетом
                AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(this); //менеджер виджетов
...
//устанавливаем содержимое соответствующих объектов интерфейса
                remoteViews.setTextViewText(R.id.widgetLangName, newItem[0]);
                remoteViews.setTextViewText(R.id.widgetLangYear, newItem[1]);
                remoteViews.setTextViewText(R.id.widgetLangPercent, newItem[2]);
                remoteViews.setImageViewResource(R.id.widgetAuthImg, R.drawable.no_picture);
//вызываем функцию setPos и устанавливаем позицию в списке виджета на 0
                NewAppWidget.setPos(0);
                remoteViews.setViewVisibility(R.id.btnPrev, View.GONE); //делаем недоступной кнопку Prev
                remoteViews.setViewVisibility(R.id.btnNext, View.VISIBLE); // активизируем кнопку Next
